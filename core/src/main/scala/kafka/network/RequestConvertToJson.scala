...
object RequestConvertToJson {
  def request(request: AbstractRequest): JsonNode = {
    request match {
      ...
      case req: GetReplicaLogInfoRequest => GetReplicaLogInfoRequestDataJsonConverter.write(req.data(), request.version())
  ...
  def response(response: AbstractResponse, version: Short): JsonNode = {
    response match {
      ...
      case res: GetReplicaLogInfoResponse => GetReplicaLogInfoResponseDataJsonConverter.write(res.data(), version)
